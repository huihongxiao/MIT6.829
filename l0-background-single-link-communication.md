...
5. 最后，很多物理介质，比如以太网，可以连接超过2台计算机。所以我们还需要考虑物理介质共享的问题。这是**media access（**注，MAC地址全称就是Media Access Control**）**或者channel access问题，它会决定在任何时刻，谁可以发送数据，以及当channel中发生竞争时，如何仲裁和解决。
...
![图1：总结了以太网的MAC协议。函数CalcBackoff会从0和当前的回退间隔中随机一个数。每次失败的传输，都会使得回退间隔加倍。](<.gitbook/assets/image (2) (1).png>)
