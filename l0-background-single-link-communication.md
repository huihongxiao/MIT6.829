...
>
> 在10Mbps网络中，64字节对应的传输时间是64\*8/10000000=51.2us，所以发送完之后会等待51.2us。
>
> 为什么不是比64字节更大的数字？因为越大传输效率越低。
>
> 为什么不是比64字节更小的数字？
>
> 在一个10Mbps，半双工的传输网络中，假设有A，B两个host。A发送Frame的起始时间是t；A的Frame的第一个bit到达B的时候是t+d；假设此时B也开始发送Frame，因为B还没收到A的电信号，所以B认为共享介质是空闲的，B也会发送；B的Frame的第一个bit到达A的时候是t+d+d；此时A才能探测到冲突，回退并重新发送Frame。所以A发送一个Frame的时间，不能少于2d的时间，换言之：Frame的长度至少要能够在共享介质中传输2d的时间。这里d就是电信号在A和B之间的共享介质中传输的时间。
>
> 通常认为电的传播速度可以认为是光速的2/3，所以d取决于共享介质，也就是线缆的长度。取64字节的Frame长度，可以跑在51.2us \* 2 \* 10^8 m/s / 2 = 5.12 km 最长长度的线缆上。降低最低Frame Size，那么能支持的最短线缆长度就会变短。早期就是需要这么长的线缆支持，所以就有了64字节这个数字。
>
